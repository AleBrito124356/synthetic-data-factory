...
Date helpers ``year(d)``, ``month(d)``, ``day(d)``, ``weekday(d)`` (Monday =
0), ``days_between(a, b)`` and ``hours_between(a, b)`` work on the ISO strings
that date/datetime fields produce, e.g. ``days_between(order_date, ship_date)``.
"""
...
def _to_date(value: Any) -> _dt.date:
    ...
    raise FormulaError(f"Expected an ISO date string, got {value!r}.")
...
def _to_datetime(value: Any) -> _dt.datetime:
    ...
    if isinstance(value, str) and len(value) <= 40:
        text = value.strip()
        return _dt.datetime.fromisoformat(text) if len(text) > 10 else \
            _dt.datetime.combine(_dt.date.fromisoformat(text), _dt.time.min)
    raise FormulaError(f"Expected an ISO date/datetime string, got {value!r}.")
...
def _hours_between(start: Any, end: Any) -> float:
    return (_to_datetime(end) - _to_datetime(start)).total_seconds() / 3600.0
...
_FUNCS: Dict[str, Callable[..., Any]] = {
    ...
    "coalesce": _coalesce,
    "is_null": _is_null,
    # dates (ISO strings, as generated by date/datetime fields)
    "year": lambda v: _to_date(v).year,
    "month": lambda v: _to_date(v).month,
    "day": lambda v: _to_date(v).day,
    "weekday": lambda v: _to_date(v).weekday(),
